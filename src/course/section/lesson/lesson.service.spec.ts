...
describe('LessonService', () => {
    ...
    describe('addLessonImage', () => {
        it('should upload image with correct key structure', async () => {
            ...
            prisma.lesson.findUnique.mockResolvedValue({ id: lessonId, slug: lessonSlug });
            s3.uploadFile.mockResolvedValue(mockUrl);
            prisma.lesson.update.mockResolvedValue({ id: lessonId, coverImage: mockUrl });

            await service.addLessonImage(file, lessonId, sectionSlug, courseSlug);

            expect(s3.uploadFile).toHaveBeenCalledWith(
                file,
                expect.stringMatching(new RegExp(`^${courseSlug}/sections/${sectionSlug}/lessons/${lessonSlug}/coverImage/\\d+_file.png$`))
            );
        });
    });
    ...
    describe('addLessonVideo', () => {
        it('should upload video with correct key structure and NOT delete if no previous source', async () => {
            ...
            prisma.section.findUnique.mockResolvedValue({ id: 'section-1', courseId: 'course-1' });
            // Mock lesson find (to get slug)
            prisma.lesson.findUnique.mockResolvedValue({ id: lessonId, slug: lessonSlug, source: null });
            s3.uploadFile.mockResolvedValue(mockUrl);
            prisma.lesson.update.mockResolvedValue({ id: lessonId, source: mockUrl } as any);

            await service.addLessonVideo(file, lessonId, sectionSlug, courseSlug);

            expect(s3.uploadFile).toHaveBeenCalledWith(
                file,
                expect.stringMatching(new RegExp(`^${courseSlug}/sections/${sectionSlug}/lessons/${lessonSlug}/video/\\d+_file.mp4$`))
            );
        ...
        it('should delete old video file if it exists', async () => {
            ...
            prisma.lesson.update.mockResolvedValue({ id: lessonId, source: newMockUrl } as any);
