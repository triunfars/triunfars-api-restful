...
describe('SectionService', () => {
    ...
    describe('addSectionImage', () => {
        it('should upload image with correct key structure', async () => {
            ...
            // Mock checkCourse
            prisma.course.findUnique.mockResolvedValue({ id: 'course-1', slug: courseSlug });
            s3.uploadFile.mockResolvedValue(mockUrl);
            prisma.section.update.mockResolvedValue({ slug: sectionSlug, coverImage: mockUrl });

            await service.addSectionImage(sectionSlug, courseSlug, file);

            expect(s3.uploadFile).toHaveBeenCalledWith(
                file,
                expect.stringMatching(new RegExp(`^${courseSlug}/sections/${sectionSlug}/coverImage/\\d+_file.png$`))
            );
        });
    });
});
